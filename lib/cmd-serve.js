...
async function serve (options, logger) {
  ...
  processingFactory.filemap = new FilemapWithAliases({ aliases, packageScope, wd })
  processingFactory.urlmap = new UrlmapWithAliases({ aliases })
  processingFactory.urlmap.cache = cache
  processingFactory.cmd = 'serve'
  processingFactory.env = createEnv()
  ...
  async function middleware (req, res) {
    ...
    const processing = processingFactory.create('.' + url, fakeReferer) // treat urls as relative paths
    if (processing != null) {
      res.writeHead(200, {
      ...
      const streams = await processing.getStreams()
      if (streams.length > 1) {
        pipeline(
          ...streams,
          err => {
