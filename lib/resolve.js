...
function resolve (to, from, options) {
  ...
  if (
    ...
    return (
      ...
      resolveAsFileSync(path.join(path.dirname(from), to), options.exts) ??
      resolveAsDirectorySync(path.join(path.dirname(from), to), options.exts) ??
      null
    )
  }
  return (
    // resolvePackageSelfSync() ??
    resolveNodeModulesSync(to, path.dirname(from), options.exts, options.cache) ??
    null
  )
}
